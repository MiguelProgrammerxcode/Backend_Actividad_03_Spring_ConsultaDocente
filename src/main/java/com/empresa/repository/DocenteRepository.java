...
public interface DocenteRepository extends JpaRepository<Docente, Integer> {
	...
	@Query("select d from Docente d where ( ?1 is '' or d.dni like ?1 ) and ( ?2 is '' or d.nombre like ?2 ) and ( ?3 is -1  or d.ubigeo.idUbigeo = ?3 ) "
			+ "and ( d.estado = ?4 ) ")
	public abstract List<Docente> listaDocentePorDniNombreUbigeo(String dni, String nombre,int idUbigeo, int estado);
